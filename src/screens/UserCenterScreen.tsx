...
import {
  ...
  LogOut, ChevronRight, Check, Copy, BadgeCheck, Star, Percent, Users, Send, Link, Trash2, Info, X, Bell, MessageSquare, HelpCircle
} from 'lucide-react';
...
export default function UserCenterScreen({ userId, profile, onBack, onLogout, onProfileUpdate }: Props) {
  ...
  const SimpleModal = ({ title, children }: { title: string; children: React.ReactNode }) => (
    <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
      <div className="bg-[#1e2026] border border-[#2b2f36] rounded-3xl w-full max-w-sm p-6 relative max-h-[80vh] overflow-y-auto">
        ...
        {tab === 'preference' && (
          <div>
            ...
            <div className="px-4 py-4 border-b border-[#1e2026] flex items-center gap-4">
              ...
              <span className="flex-1 text-sm text-[#eaecef]">Price Up Color</span>
              <button onClick={toggleColorPref} className="flex items-center gap-2">
                <span className={`w-3 h-3 rounded-full ${colorUp === 'green' ? 'bg-emerald-400' : 'bg-rose-400'}`} />
                <span className="text-sm text-[#848e9c] capitalize">{colorUp}</span>
              </button>
            </div>

            <div className="px-4 py-4 border-b border-[#1e2026] flex items-center gap-4">
              <Wallet className="w-5 h-5 text-[#848e9c] flex-shrink-0" />
              <span className="flex-1 text-sm text-[#eaecef]">Preferred Currency</span>
